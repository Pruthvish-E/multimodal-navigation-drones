...
class DeepQAgent(object):
    ...
    def __init__(self, input_shape, nb_actions,
                 ...
                 memory_size=50000, train_after=10000, train_interval=4, target_update_interval=1000,
                 monitor=True):
        ...
        if(os.path.exists("checkpoint.dnn")):
        	self._trainer.restore_from_checkpoint("checkpoint.dnn")
        	print("loaded from checkpoint")
        else:
        	self._trainer.save_checkpoint("checkpoint.dnn")        	
...
def compute_reward(quad_state, quad_vel, collision_count , destination_point,prev_dist,prev_reward):
    thresh_dist = 20
    beta = 1

    z = -10
    a = np.array(destination_point)
    b = np.array(list((quad_state.x_val, quad_state.y_val, quad_state.z_val)))
    reward = prev_reward
...
while True:
	episode_no+=1
	d_x = random.uniform(-300.0,300.0)
	d_y = random.uniform(-300.0,300.0)
	d_z = random.uniform(-10,0) 
	destination_points = [d_x,d_y,d_z]
	collision_count = 0
	dist = -1
	prev_reward = 0

	print("Started episode ",episode_no," random_destination_point : ", tuple(destination_points))
	while True:
		...
		if(collision_info.has_collided):
			collision_count+=1
		reward,dist = compute_reward(quad_state, quad_vel, collision_count,destination_points,dist,prev_reward)
		prev_reward = reward + collision_count*-10
		done = isDone(reward)
